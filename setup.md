...
あなたは、いまこのパソコンに初めて入った Claude Code です。持ち主はプログラミング未経験で、隣にはサポート役の講師がいます。
...
3. いま居るフォルダがデスクトップの `AI` フォルダで、`CLAUDE.md` `tasks.md` `SKILL_CATALOG.md` `memory/` があるか
4. `~/.claude/skills/` に `morning-briefing` `meeting-notes` `doc-draft` `learn` の4つがあるか
5. `/status` 相当の情報: ログインしているアカウントが claude.ai のサブスクリプション（Pro/Max）か。APIキー方式だと Phase B の連携が現れないので、その場合は `/login` で claude.ai アカウントに入り直すよう案内
...
4. Claude Code 側で `/mcp` を実行し、3つが `Connected`（または `cached`）で並んでいるか確認する。並んでいなければ Claude Code を一度終了して `AI秘書を起動` から入り直してもらい、再度 `/mcp`
...
3. Claude Code 側で `/chrome` を実行し、`Status: Enabled` と `Extension: Installed` になっているか確認する。なっていなければ Claude Code を一度終了して `claude --chrome` で起動し直してもらう（`AI秘書を起動` のショートカットは `claude` 起動なので、この時だけ黒い画面に `claude --chrome` と打ってもらう）
...
4. Claude Code でも使えることを確認: 黒い画面で同じように喋って、文章が入ることを確認
...
5. 最後に **セットアップ完了レポート** を出す:
   ...
   明日からは、デスクトップの「AI秘書を起動」→「おはよう」で始まります。
